...
export function SubjectsPage() {
  ...
  return (
    <div className="space-y-5">
      ...
      {loading ? <div className="p-8 text-center text-sm text-slate-400">Chargement...</div> : subjects.length === 0 ? <EmptyState icon={BookOpen} message="Aucune matière. Les matières sont générées lors de l'onboarding." /> : (
        <div className="bg-white rounded-xl border border-slate-100 shadow-sm overflow-hidden">
          <table className="w-full text-sm"><thead><tr className="border-b border-slate-100 bg-slate-50/50"><th className="text-left px-4 py-3 font-semibold text-slate-600">Matière</th><th className="text-left px-4 py-3 font-semibold text-slate-600">Code</th><th className="text-center px-4 py-3 font-semibold text-slate-600">Coefficient</th><th className="text-right px-4 py-3 font-semibold text-slate-600">Actions</th></tr></thead>
            <tbody className="divide-y divide-slate-50">{subjects.map((s) => (<tr key={s.id} className="hover:bg-slate-50/50"><td className="px-4 py-3 font-medium text-slate-900">{s.name}</td><td className="px-4 py-3 text-slate-600">{s.code || '—'}</td><td className="px-4 py-3 text-center text-slate-700">{s.coefficient}</td><td className="px-4 py-3 text-right"><div className="inline-flex gap-1"><button onClick={() => { setEditing(s); setShowForm(true); }} className="p-1.5 rounded hover:bg-slate-100 text-slate-500"><Pencil size={15} /></button><button onClick={() => remove(s.id)} className="p-1.5 rounded hover:bg-rose-50 text-rose-500"><Trash2 size={15} /></button></div></td></tr>))}</tbody></table>
        </div>
      )}
  ...
  return (
    <Modal title={subject ? 'Modifier' : 'Nouvelle matière'} onClose={onClose}>
      <form onSubmit={submit} className="space-y-4">
        ...
        <div><label className="block text-sm font-medium text-slate-700 mb-1.5">Code</label><input value={form.code} onChange={(e) => setForm({ ...form, code: e.target.value })} className={inputCls} /></div>
        <div><label className="block text-sm font-medium text-slate-700 mb-1.5">Coefficient</label><input type="number" step="0.5" value={form.coefficient} onChange={(e) => setForm({ ...form, coefficient: Number(e.target.value) })} className={inputCls} /></div>
