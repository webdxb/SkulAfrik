    ...
    <div className="space-y-5">
      ...
      {loading ? <div className="p-8 text-center text-sm text-slate-400">Chargement...</div> : events.length === 0 ? <EmptyState icon={Calendar} message="Aucun événement." /> : (
        <div className="bg-white rounded-xl border border-slate-100 shadow-sm overflow-hidden">
          <div className="divide-y divide-slate-50">{events.map((e) => (<div key={e.id} className="flex items-center justify-between p-4 hover:bg-slate-50/50"><div className="flex items-center gap-3"><div className={`rounded-lg px-2.5 py-1 text-xs font-medium ${typeColor(e.event_type)}`}>{e.event_type}</div><div><p className="font-medium text-slate-900">{e.title}</p><p className="text-sm text-slate-500">{e.start_date} {e.end_date && `→ ${e.end_date}`}</p></div></div><div className="flex gap-1"><button onClick={() => { setEditing(e); setShowForm(true); }} className="p-1.5 rounded hover:bg-slate-100 text-slate-500"><Pencil size={15} /></button><button onClick={() => remove(e.id)} className="p-1.5 rounded hover:bg-rose-50 text-rose-500"><Trash2 size={15} /></button></div></div>))}</div>
        </div>
      )}
  ...
  return (
    <Modal title={event ? 'Modifier' : 'Nouvel événement'} onClose={onClose}>
      <form onSubmit={submit} className="space-y-4">
        ...
        <div><label className="block text-sm font-medium text-slate-700 mb-1.5">Description</label><textarea value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} className={inputCls} rows={3} /></div>
