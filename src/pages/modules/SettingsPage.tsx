...
export function SettingsPage() {
  ...
  return (
    <div className="space-y-5 max-w-3xl">
      ...
      <form onSubmit={save} className="bg-white dark:bg-slate-900 rounded-xl border border-slate-100 dark:border-slate-800 shadow-sm divide-y divide-slate-100">
        <div className="p-6">
          <h3 className="font-sans font-bold text-lg text-slate-900 mb-4 flex items-center gap-2"><Building2 size={18} className="text-slate-400 dark:text-slate-500" /> Informations générales</h3>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
            <div><label className="block font-medium text-slate-700 dark:text-slate-300 mb-1.5">Nom commercial</label><input disabled={!editing} value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} className={inputErrorCls(!!errors.name)} />{editing && <FieldErrorView error={errors.name} />}</div>
            <div><label className="block font-medium text-slate-700 dark:text-slate-300 mb-1.5">Raison sociale</label><input disabled={!editing} value={form.legal_name} onChange={(e) => setForm({ ...form, legal_name: e.target.value })} className={inputErrorCls(!!errors.legal_name)} />{editing && <FieldErrorView error={errors.legal_name} />}</div>
            <div><label className="block font-medium text-slate-700 dark:text-slate-300 mb-1.5">Pays</label>
            ...
            <div><label className="block font-medium text-slate-700 dark:text-slate-300 mb-1.5">Région</label><input disabled={!editing} value={form.region} onChange={(e) => setForm({ ...form, region: e.target.value })} className={inputErrorCls(!!errors.region)} />{editing && <FieldErrorView error={errors.region} />}</div>
            <div><label className="block font-medium text-slate-700 dark:text-slate-300 mb-1.5">Ville</label><input disabled={!editing} value={form.city} onChange={(e) => setForm({ ...form, city: e.target.value })} className={inputErrorCls(!!errors.city)} />{editing && <FieldErrorView error={errors.city} />}</div>
            <div><label className="block font-medium text-slate-700 dark:text-slate-300 mb-1.5">Adresse</label><input disabled={!editing} value={form.address} onChange={(e) => setForm({ ...form, address: e.target.value })} className={inputCls} /></div>
            <div><label className="block font-medium text-slate-700 dark:text-slate-300 mb-1.5">Téléphone</label><input disabled={!editing} value={form.phone} onChange={(e) => setForm({ ...form, phone: e.target.value })} className={inputErrorCls(!!errors.phone)} placeholder="+237XXXXXXXXX" />{editing && <FieldErrorView error={errors.phone} />}</div>
            <div><label className="block font-medium text-slate-700 dark:text-slate-300 mb-1.5">Langue par défaut</label><select disabled={!editing} value={form.language} onChange={(e) => setForm({ ...form, language: e.target.value })} className={inputCls}><option value="fr">Français</option><option value="en">English</option></select></div>
          </div>
        </div>
        <div className="p-6">
          <h3 className="font-sans font-bold text-lg text-slate-900 mb-4">Responsable légal</h3>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
            <div><label className="block font-medium text-slate-700 dark:text-slate-300 mb-1.5">Nom du directeur</label><input disabled={!editing} value={form.director_name} onChange={(e) => setForm({ ...form, director_name: e.target.value })} className={inputErrorCls(!!errors.director_name)} />{editing && <FieldErrorView error={errors.director_name} />}</div>
            <div><label className="block font-medium text-slate-700 dark:text-slate-300 mb-1.5">Email du directeur</label><input disabled={!editing} value={form.director_email} onChange={(e) => setForm({ ...form, director_email: e.target.value })} className={inputErrorCls(!!errors.director_email)} />{editing && <FieldErrorView error={errors.director_email} />}</div>
          </div>
        </div>
        {editing && (
      ...
      <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-100 dark:border-slate-800 shadow-sm p-6">
        <div className="flex items-start justify-between">
          <div>
            <h3 className="font-sans font-bold text-lg text-slate-900 mb-1 flex items-center gap-2"><Lock size={18} className="text-slate-400 dark:text-slate-500" /> Devise</h3>
            <p className="text-sm text-slate-500 dark:text-slate-400">Verrouillée pour garantir la cohérence des rapports et factures.</p>
            <p className="mt-2 font-semibold text-slate-900 dark:text-slate-100">{CURRENCIES.find((c) => c.code === school?.currency)?.label || school?.currency}</p>
          </div>
          <button onClick={() => setShowCurrencyModal(true)} className="inline-flex items-center gap-1.5 rounded-lg border border-slate-200 px-3 py-1.5 text-sm font-medium text-slate-600 hover:bg-slate-50 dark:hover:bg-slate-800">Demander un changement</button>
        </div>
          ...
          <div className="mt-4 space-y-2">
            {changeRequests.map((cr) => (
              <div key={cr.id} className="flex items-center justify-between rounded-lg border border-slate-100 px-3 py-2 text-sm">
                <span className="text-slate-600 dark:text-slate-400 dark:text-slate-400">Demande vers <strong>{cr.requested_currency}</strong> — {cr.reason || 'sans motif'}</span>
      ...
      {/* Status */}
      <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-100 dark:border-slate-800 shadow-sm p-6">
        <h3 className="font-sans font-bold text-lg text-slate-900 mb-3 flex items-center gap-2"><ShieldCheck size={18} className="text-slate-400 dark:text-slate-500" /> Statut</h3>
  ...
  return (
    <Modal title="Demande de changement de devise" onClose={onClose}>
      <form onSubmit={submit} className="space-y-4">
        ...
        <div><label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1.5">Nouvelle devise</label><select value={form.requested_currency} onChange={(e) => setForm({ ...form, requested_currency: e.target.value })} className={inputCls}><option value="">Sélectionner...</option>{CURRENCIES.filter((c) => c.code !== current).map((c) => <option key={c.code} value={c.code}>{c.label}</option>)}</select></div>
        <div><label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1.5">Motif</label><textarea value={form.reason} onChange={(e) => setForm({ ...form, reason: e.target.value })} className={inputCls} rows={3} placeholder="Erreur initiale, changement de marché..." /></div>
