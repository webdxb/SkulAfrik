...
export function inputErrorCls(hasError: boolean) {
  return hasError
    ? 'w-full rounded-lg border border-rose-300 dark:border-rose-700 bg-white dark:bg-slate-800 px-3 py-2 text-sm text-slate-900 dark:text-slate-100 focus:border-rose-500 focus:ring-1 focus:ring-rose-500 outline-none'
    : inputCls;
}

export function FieldError({ error }: { error?: string | null }) {
  if (!error) return null;
  return <p className="mt-1 text-xs text-rose-600 dark:text-rose-400">{error}</p>;
}
...
export function StatCard({ label, value, color, icon: Icon }: { label: string; value: string | number; color: string; icon?: any }) {
  return (
    <div className={`bg-white dark:bg-slate-900 rounded-xl border-l-4 ${color} border-y border-r border-slate-100 dark:border-slate-800 p-5 shadow-sm`}>
      <div className="flex items-center justify-between">
        <div><p className="text-sm text-slate-500 dark:text-slate-400">{label}</p><p className="mt-1 font-heading text-2xl font-bold text-slate-900 dark:text-slate-100">{value}</p></div>
        {Icon && <div className="h-11 w-11 rounded-lg bg-slate-50 dark:bg-slate-800 flex items-center justify-center"><Icon className="text-slate-600 dark:text-slate-400" size={22} /></div>}
      </div>
    </div>
  );
}
...
export function EmptyState({ icon: Icon, title, message, action }: { icon?: any; title?: string; message?: string; action?: ReactNode }) {
  return (
    <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-100 dark:border-slate-800 p-12 text-center">
      ...
      {title && <p className="mt-3 text-sm font-medium text-slate-600 dark:text-slate-300">{title}</p>}
      {message && <p className="mt-1 text-sm text-slate-400 dark:text-slate-500">{message}</p>}
